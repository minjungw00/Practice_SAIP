...
class DualMomentum:
    ...
    def get_abs_momentum(self, rltv_momentum, start_date, end_date):
        ...
        # 사용자가 입력한 매수일을 DB에서 조회되는 일자로 보정
        sql = f"select max(date) from daily_price where date <= '{start_date}'"
        cursor.execute(sql)
        result = cursor.fetchone()
        ...
        # 사용자가 입력한 매도일을 DB에서 조회되는 일자로 보정
        sql = f"select max(date) from daily_price where date <= '{end_date}'"
        cursor.execute(sql)
        result = cursor.fetchone()
        if result[0] is None:
...
if __name__ == "__main__":
    dm = DualMomentum()
    rm = dm.get_rltv_momentum('2021-05-15', '2021-08-15', 10)
    print(rm)
    am = dm.get_abs_momentum(rm, '2021-08-15', '2021-11-15')
    print(am)